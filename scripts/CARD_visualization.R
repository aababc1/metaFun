...
# prohibit too long legned text 
arg_info$"Drug_Class" <- sapply(arg_info$"Drug_Class", wrap_after_two_semicolons)

head(arg_info)

row_annotation <- HeatmapAnnotation(df = arg_info, which='row',
                                    annotation_name_gp = gpar(fontsize = 7))
message("which")
head(row_annotation)
...
if (length(unique(as.vector(card_heatmap_data))) == 1) {
...
ht <- Heatmap(
    ...
    name = "Antimicrobial resistance gene Presence/Absence",
