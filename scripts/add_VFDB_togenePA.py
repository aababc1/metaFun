...
parser.add_argument('-g', '--gene_matrix', required=True, help='Gene presence absence or gene family count file path')
...
gene_matrix_raw = pd.read_csv(args.gene_matrix, sep='\t', header = 0) #, dtype=str)
gene_names_invfdb = diamond_output['gene_name'].unique()
#gene_names = raw_data.iloc[:, 0].unique()

#filtered_gene_matrix = gene_matrix_raw.index.isin(gene_names_invfdb)
#filtered_gene_matrix = gene_matrix_raw[gene_matrix_raw['Gene'].isin(gene_names_invfdb)]

filtered_gene_matrix = gene_matrix_raw[gene_matrix_raw['Gene'].isin(diamond_output['gene_name'])]
...
merged_data = pd.merge(filtered_gene_matrix, annotated_diamond_output[[ 'gene_name','VFG_ID', 'gene_description', 'VF_ID', 'VFC_ID', 'organism']], 
...
# final_columns = gene_matrix.columns.tolist() + ['VF_Number']
