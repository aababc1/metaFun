...
<script>
    document.addEventListener('DOMContentLoaded', (event) => {
        ...
        // Initial size setting and respond to window resize
        resizePlot();
...
for col in permanova_columns:
    ...
    if cleaned_data[col].nunique() > 1 and len(cleaned_data) >= 5:  # Minimum 5 samples required
...
for col in permanova_columns:
    create_pcoa_plot_for_column(pcoa_fig, col)
    # Hide all traces except initial column (Completeness)
    if col != initial_col:
        for trace in pcoa_fig.data[-len(cleaned_data_dict[col][col].unique()):]:
            trace.visible = False

# Show only traces for initial column (Completeness)
