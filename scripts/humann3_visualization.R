...
bcd_plot <-  bcd_plot+ theme(axis.text.x = element_text(size = 6),
  ...
  message("beta diversity group differece figure was created.")
...
glefse_3 <- tt$plot_heatmap(facet = group_col_name, xtext_keep = FALSE,
  ...
  message("Diefferential taxa analysis heatmap figure was saved. : humann_lefse_heatmap.pdf")   
}, error = function(e) {
  message("Error in aligning DA plot and relative abundance plot: ", e$message)
})
  # ggsave(file.path(output_dir, "humann_DA_function_heatmap_relab.pdf"), 
  #         glefse_3, width = 5.5, height= 6)
